...
## 2026-05-05 - DOM XSS via FastAPI Validation Errors
**Vulnerability:** A reflected DOM-based XSS existed in `app/viewer.html` because the frontend directly rendered the `.message` property of API fetch errors into `document.body.innerHTML`. FastAPI 422 Unprocessable Entity errors (from Pydantic validation) reflect the exact, unescaped raw user input provided in URL parameters.
**Learning:** Never trust error messages from APIs, even internal ones, to be XSS-safe. Frameworks like FastAPI/Pydantic deliberately echo invalid inputs in validation errors for debugging. When the frontend dynamically injects these JSON error details directly into the DOM using `innerHTML`, it creates a reflected XSS vector.
**Prevention:** Always use safe DOM APIs like `.textContent` or `innerText` when rendering dynamic error messages, or explicitly sanitize the content before using `innerHTML`.
