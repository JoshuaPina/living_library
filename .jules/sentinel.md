...
## 2024-05-31 - [DOM-based XSS in Error Rendering]
**Vulnerability:** The application was vulnerable to DOM-based Cross-Site Scripting (XSS) due to rendering raw error messages directly into the DOM using `innerHTML` in `app/viewer.html`. If an attacker crafts a malicious URL containing JavaScript, the error message returned by the backend may include this payload, executing it in the victim's browser.
**Learning:** Directly interpolating user-controlled data or untrusted backend responses (like error messages) into `innerHTML` is dangerous. Modern frameworks handle this implicitly, but vanilla JS requires strict discipline to separate content from HTML structure.
**Prevention:** Always use safe DOM APIs like `textContent` or `innerText` when inserting text into the page, especially for dynamic error messages or data reflected from the URL. Avoid `innerHTML` unless rendering explicitly trusted, sanitized HTML content.
