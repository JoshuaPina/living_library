...
## 2024-05-31 - [Reflected DOM-based XSS via Error Messages]
**Vulnerability:** The frontend application in `assets/scripts.js` caught a `fetch` exception and directly rendered `error.message` into the DOM using `.innerHTML`. Since the exception message included unescaped portions of the raw server response body (which itself could reflect user input like invalid search queries), this created a reflected DOM-based Cross-Site Scripting (XSS) vulnerability.
**Learning:** Any data originating from an external source, including the body of an HTTP error response, must be considered untrusted. Assigning untrusted strings to `.innerHTML` exposes the application to XSS attacks.
**Prevention:** When updating the DOM with potentially untrusted text (like API error messages), prefer assigning to `.textContent` or using `document.createTextNode()`. This ensures the browser treats the input strictly as text, preventing script execution. If HTML formatting is absolutely necessary, sanitize the input explicitly before insertion.
