...
## 2026-04-03 - DOM XSS via Unescaped FastAPI/Pydantic Validation Errors
**Vulnerability:** A reflected DOM-based Cross-Site Scripting (XSS) vulnerability was found in the frontend where raw API error messages were dynamically injected directly into the DOM using `.innerHTML`. Since FastAPI/Pydantic returns HTTP 422 errors containing the raw, unescaped user input that failed validation, this allowed arbitrary script execution if a malicious payload was provided.
**Learning:** Error messages returned from an API, especially automated validation errors that mirror user input, must be treated as untrusted data. Using `.innerHTML` to render such dynamic data without escaping opens up serious XSS risks on the frontend.
**Prevention:** Always use safe DOM manipulation methods like `.textContent` when rendering dynamic error messages or data received from an API to prevent HTML injection and XSS.
