...
## 2026-05-20 - [DOM-based XSS via Error Messages]
**Vulnerability:** The application was vulnerable to DOM-based XSS when displaying backend error messages, specifically FastAPI 422 validation errors, which reflected unescaped user inputs. The vulnerability existed in both `app/viewer.html` and `assets/scripts.js` where `error.message` was directly assigned to `innerHTML`.
**Learning:** Error messages retrieved from external endpoints should always be treated as untrusted input. In particular, Pydantic's 422 Unprocessable Entity responses reflect the exact raw data submitted by the client within the error detail. Directly passing this into `innerHTML` allows script execution.
**Prevention:** When displaying potentially unsafe error messages in the DOM, always use `.textContent` instead of `.innerHTML`, or manually sanitize the message using an encoding function (e.g., `escapeHTML`) before injecting it.
