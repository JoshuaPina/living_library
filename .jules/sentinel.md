...
## 2024-05-18 - [Fix DOM-based XSS in Error Handling]
**Vulnerability:** A DOM-based XSS vulnerability existed in `app/viewer.html` because the `materialId` taken from URL parameters was used in an API fetch, and when the API returned an error (such as a 422 JSON validation error containing the raw malicious payload), the unescaped error message (`error.message`) was directly inserted into the DOM via `document.body.innerHTML = ...`.
**Learning:** Even if the backend API strictly enforces types (e.g., expecting an integer), Pydantic's automatic 422 Unprocessable Entity responses include the rejected input. When the frontend recklessly interpolates this reflected input directly into HTML strings, it leads to reflected DOM-based XSS. We cannot trust backend error messages to be HTML-safe, especially when they reflect user input.
**Prevention:** Always sanitize dynamically rendered text before inserting it into the DOM. Use a helper function like `escapeHTML` to encode characters when `innerHTML` is necessary, or prefer using safe assignments like `.textContent` or `.innerText` which natively prevent script injection.
