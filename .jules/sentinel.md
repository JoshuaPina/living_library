...
## 2024-05-31 - [DOM-based XSS in Error Messages]
**Vulnerability:** The frontend application (`app/viewer.html` and `assets/scripts.js`) was dynamically rendering backend error payloads directly into the DOM using template literals and `.innerHTML` (e.g., `container.innerHTML = \`<p>${error.message}</p>\``).
**Learning:** If an API endpoint fails and returns an unescaped error message containing malicious HTML or JavaScript, directly assigning that message to `.innerHTML` creates a reflected DOM-based Cross-Site Scripting (XSS) vulnerability.
**Prevention:** Always explicitly sanitize dynamic text before insertion, or construct the static HTML structure with `.innerHTML` first, and then safely inject the dynamic text payload into the specific node using `.textContent` (e.g., `container.querySelector('p').textContent = error.message`).
