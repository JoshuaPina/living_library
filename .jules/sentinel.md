...
## 2024-06-05 - [Reflected DOM XSS via Error Handling]
**Vulnerability:** The frontend application displayed error messages originating from the backend directly using `.innerHTML` (e.g., `document.body.innerHTML = \`<p>${error.message}</p>\`;`).
**Learning:** If an API endpoint reflects unescaped user input in its error responses (such as FastAPI's default 422 validation errors), rendering those messages with `.innerHTML` creates a DOM-based XSS vulnerability. This shows that even API error messages must be treated as untrusted data by the frontend.
**Prevention:** Always use `.textContent`, `document.createTextNode()`, or a dedicated sanitization function like `escapeHTML` when updating the DOM with potentially dynamic text, including error messages from API responses.
