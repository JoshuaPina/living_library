...
## 2024-05-31 - [Reflected XSS in Error Messages]
**Vulnerability:** The frontend application (`app/viewer.html` and `assets/scripts.js`) caught generic fetch exceptions and dynamically rendered the `.message` property of the Error object directly into the DOM using `.innerHTML`. This created a DOM-based Reflected XSS vulnerability, particularly if backend APIs return unescaped user input inside validation error messages.
**Learning:** Using `.innerHTML` to insert text that originates from an API response (even if wrapped in an Error object) is unsafe. Attackers can craft malicious URLs or inputs that force the backend to reflect the input in an error response, which is then executed by the browser.
**Prevention:** Always use `.textContent` or `document.createTextNode()` when dynamically inserting text content, such as error messages, into the DOM to ensure the browser treats it strictly as text and not as executable HTML.
