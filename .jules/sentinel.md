...
## 2024-06-03 - [DOM-based XSS via Reflected Error Messages]
**Vulnerability:** The frontend application was directly interpolating unescaped error messages (e.g., `error.message` and `errorMsg`) into the DOM using `.innerHTML`. Since FastAPI/Pydantic returns the raw user input within validation error messages (HTTP 422), an attacker could inject malicious scripts by crafting URLs with XSS payloads that fail validation.
**Learning:** Directly rendering error messages or variables derived from HTTP responses into `.innerHTML` creates a DOM-based XSS vulnerability, especially since backend validation errors frequently reflect user input directly.
**Prevention:** Always use a sanitization function like `escapeHTML()` to sanitize strings before interpolating them into `.innerHTML`, or assign text content directly to `.textContent` instead of using `.innerHTML`.
