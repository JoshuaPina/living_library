...
## 2025-02-21 - Fix DOM-based XSS in Error Handling
**Vulnerability:** API error messages were returned unescaped and injected directly into the DOM using `.innerHTML` (e.g., `document.body.innerHTML = \`<p>${error.message}</p>\``). This exposed a DOM-based Cross-Site Scripting (XSS) vulnerability.
**Learning:** Any backend-generated error text (especially from API failures or explicit server strings) that eventually reaches the client UI must be considered untrusted data and sanitized prior to injection into the DOM.
**Prevention:** Always use `.textContent` instead of `.innerHTML` when displaying error strings, or proactively sanitize the string using an `escapeHTML` helper function before interpolation.
