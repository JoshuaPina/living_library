...
## 2024-05-24 - Fix Reflected DOM-based XSS via Error Messages
**Vulnerability:** The application was vulnerable to DOM-based XSS where error messages generated dynamically by FastAPI endpoints (like 422 Unprocessable Entity payloads returning raw user input) were inserted without sanitization into `innerHTML` properties across both `app/viewer.html` and `assets/scripts.js`.
**Learning:** When using modern APIs, any raw string dynamically generated in an error state - even if from a framework like FastAPI - can contain arbitrary user payloads. Directly assigning these to `innerHTML` is inherently unsafe.
**Prevention:** Either explicitly sanitize any string coming from backend error handling before appending it via `innerHTML`, or prefer `.textContent` assignment to naturally prevent JS execution payloads.
