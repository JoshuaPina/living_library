...
## 2024-06-05 - [DOM-based XSS via Error Messages]
**Vulnerability:** The frontend application dynamically rendered backend error messages directly into the DOM using `innerHTML` (e.g., `document.body.innerHTML = \`<p>${error.message}</p>\``).
**Learning:** This exposes the application to reflected DOM-based XSS vulnerabilities if an attacker can manipulate the error response payload or its trigger conditions. FastAPI/Pydantic validation errors (HTTP 422) automatically reflect raw, unescaped user input in their JSON response payloads.
**Prevention:** Always use safe DOM assignment methods like `.textContent` when inserting untrusted data, including error messages from backend APIs. Construct safe HTML templates first, then inject text content.
