...
## 2024-05-31 - [Reflected DOM XSS via Validation Errors]
**Vulnerability:** The application was vulnerable to DOM-based Cross-Site Scripting (XSS) because FastAPI validation error messages (like those resulting from 422 HTTP responses) were directly injected into the DOM via `innerHTML` without sanitization. If user input was reflected in the validation error, it could execute malicious scripts.
**Learning:** Automatically rendering backend error messages directly using `innerHTML` is dangerous, particularly when validation errors reflect raw, unescaped user input (e.g., Pydantic displaying what failed validation).
**Prevention:** When displaying dynamic error messages in the DOM, always use safe properties like `textContent` or `innerText`, or explicitly sanitize the input using a function like `escapeHTML` if `innerHTML` is strictly necessary.
